      ...
      <div className="space-y-3">
        {links.map((link, i) => (
          <div key={i} className="space-y-3 rounded-lg border border-slate-200 p-4">
            ...
            {isUnknownInternalPath(link.href) && (
              <p className="rounded-lg bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-700">
                No page exists at this path, so the site hides this link. Use a
                full https:// address for an external page, or ask for the page
                to be built.
              </p>
            )}
            <LangRow
